...
const vpn2 = document.getElementById('vpn2');
	...
	if (user.displayName && user.email) {
		...
		if(user.email.includes('yahoo.com')){
			...
			if(!localStorage.getItem('verify-7')) {
				...
				vpn.removeAttribute('data-bs-toggle');
		...
		if(!localStorage.getItem('verify-7')) {
			vpn.innerHTML = `Verify Email <img src="img/partners/gmail.png">`;
			vpn.removeAttribute('data-bs-toggle');
			vpn.addEventListener('click', sendEmail);
		} else {
			vpn.innerHTML = `View Profile <img src="img/partners/gmail.png">`;
		}
		...
		vpn.innerHTML = `Email Invoice <img src="img/partners/gmail.png">`;
...
function sendEmail() {
	...
	localStorage.setItem('verify-7', true);
	vpn.innerHTML = `View Profile <img src="img/partners/google.png">`;
	vpn.setAttribute('data-bs-toggle', 'modal');
}
...
.then(function(data) {
	...
	document.getElementById('label-ip').innerHTML = `
		IP address: <span>${data.ip}</span> ${data.country_calling_code} <img src="https://flagcdn.com/144x108/${newCode}.png" id="the-flag2" />
	`;
